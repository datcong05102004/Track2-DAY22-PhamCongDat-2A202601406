...
# RAGAS 0.4.x still imports the legacy LangChain Vertex AI module.  LangChain
# 1.x moved it to a separate integration package; the placeholder is enough
# for RAGAS's optional isinstance check when this lab uses OpenAI instead.
try:
    from langchain_community.chat_models.vertexai import ChatVertexAI  # noqa: F401
except ModuleNotFoundError:
    legacy_vertexai = types.ModuleType("langchain_community.chat_models.vertexai")
    legacy_vertexai.ChatVertexAI = type("ChatVertexAI", (), {})
    sys.modules[legacy_vertexai.__name__] = legacy_vertexai
...
SYSTEM_V1 = (
    "You are a helpful AI assistant. Answer only from the supplied context. "
    "Keep the answer clear, friendly, and concise in 2 to 4 sentences. "
...
SYSTEM_V2 = (
    "You are an expert AI analyst. Use only facts in the supplied context; do not "
    "invent information. Give a well-organized answer in 3 to 5 sentences: start "
    "with the direct answer, then explain the most relevant supporting details, and "
